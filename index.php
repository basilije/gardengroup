...
<body>
    <i>__________  something! ...</i>
    <h1 style="color:green;">         Magic Garden    </h1>  
		
    <h4>         click on the button to feed the plant    </h4> 

    <form method="post"> <input type="submit" style="height:33px;" name="B1" value="Turn ON Switch 1 for"/> 
    <input type = "Text" style="height:33px;"value ="" name = "duration1"> seconds
    </form>
    
    <form method="post"> <input type="submit" style="height:33px;" name="B2" value="Turn ON Switch 2 for"/> 
    <input type = "Text" style="height:33px;" value ="" name = "duration2"> seconds
    </form>    
    
      <br></br>
      <form method="post"> 
		<select name="analog_inputs" style="height:33px;">
		...
		<input type = "Text" style="height:33px;" name = "duration3">
		values <input type="submit" style="height:33px;" name="B3">
		
		<input type="checkbox" style="height:33px;"id="to_refresh" name="to_refresh" value="on" checked> (refresh)
		</form> 
    <h4>         settings    </h4> 
    <?php
    global $to_read;
    ...
    $config_file_name = "/home/pi/magicgarden/cfg.cfg";
	if(isset($_POST['html'])) {
    ...
    $content = file_get_contents($config_file_name);
    echo '<textarea name="html">' . htmlspecialchars($content) . "</textarea>";
    echo ' <input type="submit" id="E1" name="E1" value="Edit page" />';

	require_once("phpChart_Lite/conf.php");
	if(isset($_POST['B1'])) { 
		$filename1 = "/home/pi/magicgarden/20.txt";
	...
	if(isset($_POST['B2'])) { 
		$filename2 = "/home/pi/magicgarden/18.txt";
	...
	try {
		...
		$filename = "/home/pi/magicgarden/dbs/".date("Y-m-d").".db";
	...
	$pc4->set_axes(array(
			'xaxis'=> array('label'=> 'Last '.$until_element.' Values'),
